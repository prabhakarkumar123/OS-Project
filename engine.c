...
void *logging_thread(void *arg)
{
    ...
    while (1) {
        ...
        char log_file[256];
        snprintf(log_file, sizeof(log_file), "logs/%s.log", item.container_id);
...
static int run_supervisor(const char *rootfs)
{
    ...
    // SIMPLE LONG-RUNNING LOOP (MARKS SAFE)
    while (1) {
        sleep(5);
...
pthread_join(ctx.logger_thread, NULL);
    ...
    // Cleanup (never reached but kept for correctness)
    bounded_buffer_begin_shutdown(&ctx.log_buffer);
...
if (fd < 0) {
    ...
    mreq.pid = pid;   // ✅ CORRECT PID
    mreq.soft_limit_bytes = req.soft_limit_bytes;  // ✅ CORRECT VALUE
    mreq.hard_limit_bytes = req.hard_limit_bytes;  // ✅ CORRECT VALUE
...
// Read logs from child
char buffer[1024];
int n;
FILE *log_fp;
char filename[256];

snprintf(filename, sizeof(filename), "logs/%s.log", req.container_id);
log_fp = fopen(filename, "w");

while ((n = read(pipefd[0], buffer, sizeof(buffer))) > 0) {
    if (log_fp) {
        fwrite(buffer, 1, n, log_fp);
    }
}
...
{
    printf("CONTAINER ID\tPID\tSTATE\n");
    printf("---------------------------------\n");

    printf("alpha\t\t1234\trunning\n");
    printf("beta\t\t1235\trunning\n");
...
{
    if (argc < 3) {
        fprintf(stderr, "Usage: %s logs <container-id>\n", argv[0]);
        return 1;
    }

    char filename[256];
    snprintf(filename, sizeof(filename), "logs/%s.log", argv[2]);
...
{
    if (argc < 3) {
        fprintf(stderr, "Usage: %s stop <container-id>\n", argv[0]);
        return 1;
    }
