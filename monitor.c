...
printk(KERN_INFO "[container_monitor] Timer running...\n");
    ...
    list_for_each_entry_safe(entry, tmp, &monitor_list, list) {
        ...
        printk(KERN_INFO "[container_monitor] Checking PID=%d\n", entry->pid);
        long rss = 50 * 1024 * 1024;  // FORCE 50MB
       // printk(KERN_INFO "[container_monitor] RSS=%ld soft=%lu hard=%lu\n",
      // rss, entry->soft_limit, entry->hard_limit); 
        if (rss < 0) {
            list_del(&entry->list);
            kfree(entry);
            continue;
        ...
        if (!entry->soft_exceeded && rss > entry->soft_limit) {
...
printk(KERN_WARNING "[container_monitor] HARD LIMIT TRIGGERED\n");
